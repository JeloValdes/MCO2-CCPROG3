...
public class VendingController {
    ...
    public VendingController(MainFrame frame) {
        ...
        this.ramenPanel.addLoadPresetListener(event -> loadPreset());
    ...
    /** Rebuilds the recipe dropdown, marking recipes the machine cannot assemble. */
    private void refreshPresetOptions() {
        SpecialVendingMachine special = (SpecialVendingMachine) this.machine;
        ArrayList<RamenPreset> presets = special.getPresets();
        String[] labels = new String[presets.size()];

        for (int i = 0; i < presets.size(); i++) {
            RamenPreset preset = presets.get(i);
            labels[i] = preset.getName() + " - Php " + special.getPresetPrice(preset)
                    + " - " + preset.getDescription();

            if (!special.isPresetAvailable(preset)) {
                labels[i] += "  [unavailable]";
            }
        }
        this.ramenPanel.setPresetOptions(labels);
    }

    /** Replaces the bowl with the chosen ready-made recipe. */
    private void loadPreset() {
        SpecialVendingMachine special = (SpecialVendingMachine) this.machine;
        ArrayList<RamenPreset> presets = special.getPresets();
        int index = this.ramenPanel.getSelectedPreset();

        if (index < 0 || index >= presets.size()) {
            this.ramenPanel.appendOutput("Choose a recipe from the list first.");
            return;
        }

        RamenPreset preset = presets.get(index);
        if (!special.isPresetAvailable(preset)) {
            this.ramenPanel.appendOutput(preset.getName()
                    + " cannot be made right now. Some of its ingredients have run out.");
            return;
        }
        ...
        for (int i = 0; i < names.size(); i++) {
            this.bowlSlotNumbers.add(this.machine.findSlotNumberByName(names.get(i)));
            this.bowlQuantities.add(quantities.get(i));
        }

        this.ramenPanel.appendOutput("Loaded " + preset.getName()
                + ". Adjust it if you like, then press Prepare and buy.");
        refreshRamen();
    }
