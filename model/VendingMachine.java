...
public abstract class VendingMachine {
    ...
    /** Pieces at or below this count raise a low-denomination alert. */
    public static final int LOW_DENOMINATION_LEVEL = 5;

    /** A change fund below this value raises a machine-wide alert. */
    public static final int LOW_FUND_VALUE = 200;

    /** Only the small denominations are checked for change-making capacity. */
    private static final int CHANGE_DENOMINATION_COUNT = 5;
     ...
     * post: the machine holds its default slots and every slot's summary baseline has been recorded.
     */
    ...
    /**
     * Finds the one-based position of the slot holding an item.
     *
     * @param itemName the item name to look for, ignoring capitalisation
     * @return the position counting from one, or -1 if the machine does not
     *         stock that item
     */
    public int findSlotNumberByName(String itemName) {
        if (itemName == null) {
            return -1;
        }
        for (int i = 0; i < this.slots.size(); i++) {
    ...
    /**
     * Reports whether the machine meets the minimum slot count required by the specifications.
     *
     ...
     * @return true if the item was dispensed; call {@link #getLastStatus()} for the reason when it was not
     */
     ...
     * @param insertedCash the pieces the customer just inserted, which the machine may also use to pay the change
     ...
     * post: every slot's starting stock equals its current stock and every usage counter is zero.
     */
    ...
    /**
     * Inspects the machine and reports everything an operator should attend to.
     *
     * <p>Only figures are reported, never wording, so the view decides how each
     * problem is phrased. Nothing about the machine is changed by this call.</p>
     *
     * @return the problems found, critical ones first
     */
    public ArrayList<Alert> getAlerts() {
        ArrayList<Alert> critical = new ArrayList<Alert>();
        ArrayList<Alert> warnings = new ArrayList<Alert>();

        if (this.slots.size() < MIN_SLOTS) {
            critical.add(new Alert(AlertType.BELOW_MINIMUM_SLOTS,
                    getMachineName(), this.slots.size(), MIN_SLOTS));
        }

        for (int i = 0; i < this.slots.size(); i++) {
            ItemSlot slot = this.slots.get(i);

            if (slot.isEmpty()) {
                critical.add(new Alert(AlertType.SLOT_EMPTY,
                        slot.getItemName(), 0, slot.getCapacity()));
            } else if (slot.getQuantity() <= LOW_STOCK_LEVEL) {
                warnings.add(new Alert(AlertType.SLOT_LOW,
                        slot.getItemName(), slot.getQuantity(), LOW_STOCK_LEVEL));
            }
        }

        if (getMoneyTotal() < LOW_FUND_VALUE) {
            critical.add(new Alert(AlertType.CHANGE_FUND_LOW,
                    getMachineName(), getMoneyTotal(), LOW_FUND_VALUE));
        }

        // Only the small denominations matter for making change; nobody pays
        // change in 500s, so an empty 500 tray is not a problem.
        for (int i = 0; i < CHANGE_DENOMINATION_COUNT; i++) {
            int held = this.money[i].getQty();

            if (held == 0) {
                critical.add(new Alert(AlertType.DENOMINATION_EMPTY,
                        String.valueOf(DENOMINATIONS[i]), 0, LOW_DENOMINATION_LEVEL));
            } else if (held <= LOW_DENOMINATION_LEVEL) {
                warnings.add(new Alert(AlertType.DENOMINATION_LOW,
                        String.valueOf(DENOMINATIONS[i]), held, LOW_DENOMINATION_LEVEL));
            }
        }

        critical.addAll(warnings);
        return critical;
    }
