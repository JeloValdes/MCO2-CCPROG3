...
public class RamenPanel extends JPanel {
    ...
    public RamenPanel() {
        ...
        JPanel presetRow = new JPanel(new FlowLayout(FlowLayout.LEFT, 6, 6));
        presetRow.setBackground(UI.BACKGROUND);
        this.presetBox = new JComboBox<>(new String[]{"(no recipes loaded)"});
        this.presetBox.setPreferredSize(new Dimension(320, 26));
        this.loadPresetButton = UI.button("Load recipe into bowl");
        presetRow.add(new JLabel("Chef's recipes:"));
        presetRow.add(this.presetBox);
        presetRow.add(this.loadPresetButton);
        ...
        JPanel rows = new JPanel(new java.awt.GridLayout(2, 1));
        rows.setBackground(UI.BACKGROUND);
        rows.add(presetRow);
        rows.add(controls);
        south.add(rows, BorderLayout.NORTH);
    ...
    /**
     * Replaces the list of ready-made recipes offered to the customer.
     *
     * @param labels one label per recipe, in the machine's preset order
     */
    public void setPresetOptions(String[] labels) {
        this.presetBox.removeAllItems();
        for (String label : labels) {
            this.presetBox.addItem(label);
        }
        this.loadPresetButton.setEnabled(labels.length > 0);
    }

    /**
     * Returns the recipe currently chosen.
     *
     * @return the selected preset index, or -1 if the list is empty
     */
    public int getSelectedPreset() {
        return this.presetBox.getSelectedIndex();
    }

    /**
     * Registers the handler for loading the chosen recipe into the bowl.
     *
     * @param listener the handler to attach
     */
    public void addLoadPresetListener(ActionListener listener) {
        this.loadPresetButton.addActionListener(listener);
    }
    ...
    public void setControlsEnabled(boolean enabled) {
        ...
        this.loadPresetButton.setEnabled(enabled);
    }
    ...
    /**
     * Returns the running price and calorie line as it is displayed.
     *
     * @return the totals text
     */
    public String getTotalsText() {
        return this.totalsLabel.getText();
    }

    /**
     * Reports whether the bowl controls are currently usable.
     *
     * @return true if the customer can change or buy the bowl
     */
    public boolean isControlsEnabled() {
        return this.prepareButton.isEnabled();
    }

    /**
     * Returns how many components are listed in the bowl.
     *
     * @return the row count of the bowl table
     */
    public int getBowlRowCount() {
        return this.bowlTable.getRowCount();
    }
