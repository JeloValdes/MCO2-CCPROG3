...
public class SpecialVendingMachine extends VendingMachine {
    ...
    private ArrayList<RamenPreset> presets;
    ...
    public SpecialVendingMachine() {
        ...
        this.presets = new ArrayList<RamenPreset>();
        this.ramenRevenue = 0;
        this.ramenCount = 0;
    }
    ...
    /**
     * Stocks the machine's list of ready-made recipes.
     *
     * post: the preset list matches the default stock loaded alongside it.
     */
    public void loadDefaultPresets() {
        this.presets = new ArrayList<RamenPreset>();

        RamenPreset tonkotsu = new RamenPreset("Tonkotsu Classic",
                "Rich pork broth, double chashu, soft egg");
        tonkotsu.addIngredient("Noodles", 1);
        tonkotsu.addIngredient("Chashu Pork", 2);
        tonkotsu.addIngredient("Aji Tamago", 1);
        tonkotsu.addIngredient("Tonkotsu Broth", 1);
        this.presets.add(tonkotsu);

        RamenPreset miso = new RamenPreset("Miso Veggie",
                "Miso broth with tofu and spring onion, no meat");
        miso.addIngredient("Noodles", 1);
        miso.addIngredient("Fried Tofu", 2);
        miso.addIngredient("Negi", 1);
        miso.addIngredient("Miso Broth", 1);
        this.presets.add(miso);

        RamenPreset shio = new RamenPreset("Shio Light",
                "Clear salt broth, fish cake, spring onion");
        shio.addIngredient("Noodles", 1);
        shio.addIngredient("Fish Cake", 1);
        shio.addIngredient("Negi", 1);
        shio.addIngredient("Shio Broth", 1);
        this.presets.add(shio);

        RamenPreset garlic = new RamenPreset("Black Garlic Special",
                "Double noodles finished with black garlic oil");
        garlic.addIngredient("Noodles", 2);
        garlic.addIngredient("Chashu Pork", 1);
        garlic.addIngredient("Tonkotsu Broth", 1);
        garlic.addIngredient("Black Garlic Oil", 1);
        this.presets.add(garlic);
    }

    /**
     * Returns the ready-made recipes this machine offers.
     *
     * @return a copy of the preset list
     */
    public ArrayList<RamenPreset> getPresets() {
        return new ArrayList<RamenPreset>(this.presets);
    }

    /**
     * Reports whether a recipe can be assembled from current stock.
     *
     * @param preset the recipe to check
     * @return true if every component is stocked in the quantity the recipe needs
     */
    public boolean isPresetAvailable(RamenPreset preset) {
        if (preset == null || preset.size() == 0) {
            return false;
        }

        ArrayList<String> names = preset.getIngredientNames();
        ArrayList<Integer> quantities = preset.getQuantities();

        for (int i = 0; i < names.size(); i++) {
            ItemSlot slot = findSlotByName(names.get(i));

            if (slot == null || !slot.getIngredientType().isRamenIngredient()) {
                return false;
            }
            if (slot.getQuantity() < quantities.get(i)) {
                return false;
            }
        }
        return true;
    ...
    /**
     * Returns the price a recipe would cost at the machine's current prices.
     *
     * @param preset the recipe to price
     * @return the total price in pesos, or 0 if any component is missing
     */
    public int getPresetPrice(RamenPreset preset) {
        ArrayList<String> names = preset.getIngredientNames();
        ArrayList<Integer> quantities = preset.getQuantities();
        int total = 0;
        ...
        for (int i = 0; i < names.size(); i++) {
            ...
            total += slot.getPrice() * quantities.get(i);
        }
        return total;
    }
