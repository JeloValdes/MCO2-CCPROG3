 ...
 *
 * <p>The customer picks ingredients on the left, watches the bowl take shape on
 * the right with a running price and calorie count, and then has the machine
 * prepare it. The preparation narration is fed in one line at a time by the
 * controller so the simulation shows its internal work.</p>
 */
public class RamenPanel extends JPanel {
    ...
    /**
     * Enables or disables the controls that change or buy the bowl, so they
     * cannot be used while a preparation is being narrated.
     *
